...
from models import (
    ...
    ApiKeyUpdate, users_collection, UserApiKeys, GoogleOAuthUser
)
from auth import (
...
app = FastAPI(title="German Letter AI Assistant with Multi-LLM Support and Google OAuth")
...
@app.post("/api/analyze-file-with-user-keys", response_model=LetterAnalysisResponse)
async def analyze_file_with_user_keys(
    file: UploadFile = File(...),
    language: str = Form(default="en"),
    current_user: dict = Depends(get_current_user_token)
):
    """Analyze file using user's personal API keys"""
    try:
        # Get user's API keys
        user = await users_collection.find_one({"id": current_user["sub"]})
        if not user:
        ...
        # For now, fallback to default analysis
        ...
        logger.error(f"Error analyzing file with user keys: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to analyze file: {str(e)}")
